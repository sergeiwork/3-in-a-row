...
- Menus and cards use the mapped Kenney RPG panel and nine-sliced button primitives, including pressed and disabled states. The region map is an icon-led connected route tree: paths are drawn explicitly, nodes expose stage/type/content, and available, completed, and locked states remain distinguishable without color alone.
