...
- The canvas renders at 1080×1920 by default but UI Toolkit scales from a 720×1280 logical reference and applies the device safe area. Portrait displays balance width and height scaling, while landscape desktop displays match the reference height so vertically composed screens retain their layout; the title and region map scroll vertically when their content still exceeds the viewport. This keeps live text and touch targets readable on dense mobile displays and prevents Windows widescreen layouts from flex-compressing controls into overlaps. The square board is capped independently of screen height so both 16:9 and 20:9 portrait layouts retain the enemy intent, resources, active controls, and generous cell hit regions.
...
- Encounter presentation uses a compact enemy stage, a high-contrast intent strip, a resource rail, and two ability controls around the board. Board width and height use the same measured side from remaining safe-area space, keeping cells square. Obsidian/teal crystalline ornament is decorative and ignores picking; game buttons retain pressed, focus, and disabled states. Reduced Motion disables decorative UI transitions.
