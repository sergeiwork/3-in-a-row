...
- Board motion connects a 170 ms swap to a 160 ms clear with brief anticipation, then distance-scaled 180–340 ms falls and a 75 ms landing settle. Gravity uses an asymmetric easing curve with restrained stretch and squash; positions land exactly in the destination cell and visual transforms reset before reuse. There are no per-resource/status/damage waits or unconditional batch-end pause; only the enemy intent wind-up and defeat beat receive a 180 ms hold in normal motion mode.
