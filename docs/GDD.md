...
- Title exposes Start Run, Resume when a compatible stable checkpoint exists, How to Play, Settings, device-local reduced-motion/SFX/music toggles, build version, and required asset credits. A persistent next-difficulty card states the exact unlock condition, reward rule, and current progress; before tier 1 it shows the four dominant-damage branches independently, and from tier 1 onward it names the standard-run tier that must be defeated. Run selection repeats the card beneath the available difficulty buttons, and identifies the weekly challenge as fixed at tier 3 and ineligible for difficulty-ladder progress.
...
- Tapping the currently targeting active skill again cancels target selection without using the skill or spending a turn. Cancellation clears selected targets, any chosen color, and targeting controls; tapping a different skill first clears the previous targeting mode. The explicit Cancel button follows the same behavior for every targeted active.
...
- Every level-up reward card visibly labels its definition's slot type as Active Skill or Passive Upgrade before opening details, using distinct accent colors as a secondary cue.
...
- Stable in-run screens expose a Menu control. It confirms that the latest safe checkpoint will be retained, calls `ReturnToTitle(false)`, and lets Title Resume restore the run. Presentation refuses this exit while combat input is locked or an enemy response is pending, so it cannot serialize or expose a half-resolved turn.
- Victory and defeat summaries display encounters cleared, largest cascade, resolved turns, total damage, damage by source, and chosen upgrades. They also identify goals completed during the run, rewards unlocked, dominant branches credited toward tier 1, any newly unlocked difficulty and its added rule, and the next difficulty condition. Development builds also display the run seed.

#### Responsive and accessible presentation

- The canvas renders at 1080×1920 by default but UI Toolkit scales from a 720×1280 logical reference and applies the device safe area. Portrait displays balance width and height scaling, while landscape desktop displays match the reference height. Every vertically composed screen can scroll when its content exceeds the viewport, keeping live text and touch targets readable without flex-compressing controls into overlaps.
- Responsive presentation has three deterministic layout profiles: compact/short portrait, standard portrait, and wide landscape. Safe-area padding and maximum content width come from the active profile. Information-heavy screens own exactly one top-anchored vertical scroll body, with only essential navigation or confirmation actions allowed in a fixed footer. Scrollbars remain hidden while wheel, drag, and touch scrolling stay enabled. Buttons and card copy use content-driven minimum heights and wrapping instead of fixed text heights; multi-column menus and tabs use explicit measured rows rather than overflowing flex-wrap rows. Codex content is separated into Goals, Codex, Records, and Runs tabs; Expeditions are separated into Daily, Trials, and Weekly tabs so adding entries does not lengthen unrelated sections.
- Encounter presentation uses the same scroll-body contract as other dense screens. Its board side is always square, never wider than its content area, capped at 720 logical pixels, and given a 392 logical-pixel comfort floor when width permits; a short viewport scrolls the surrounding HUD instead of compressing or overlapping intent, resource, skill, targeting, or message controls. Layout-policy Edit Mode tests cover 16:9 portrait, tall 20:9 portrait, short portrait, and desktop landscape viewports.
- Menus and cards use the mapped Kenney RPG panel and nine-sliced button primitives, including pressed and disabled states. The region map is an icon-led connected route tree: paths are drawn explicitly, nodes expose stage/type/content, and available, completed, and locked states remain distinguishable without color alone. Optional route-vow choices place the vow name and condition on separate wrapped lines so both remain inside their buttons at the 720-wide logical reference.
- Encounter presentation uses a compact enemy stage, a high-contrast intent strip, a resource rail, and two ability controls around the board. Board width and height use the same responsive-policy side, keeping cells square while the surrounding HUD scrolls on short viewports. Obsidian/teal crystalline ornament is decorative and ignores picking; game buttons retain pressed, focus, and disabled states. Reduced Motion disables decorative UI transitions.
...
- Board motion connects a 220 ms swap to a 210 ms clear with restrained anticipation, then distance-scaled 230–420 ms falls and a 130 ms landing settle. Swap, clear, gravity, and landing curves meet at zero velocity; stretch, squash, and impact scaling stay subtle while positions land exactly in the destination cell and visual transforms reset before reuse. There are no per-resource/status/damage waits or unconditional batch-end pause; only the enemy intent wind-up and defeat beat receive a 180 ms hold in normal motion mode.
- Non-interactive particles, damage numbers, and cascade banners use a separate feedback layer and finish across HUD rebuilds without delaying input or the automatic enemy response. Damage health values still update at the ordered event, and all gameplay motion completes before control returns. Reduced Motion reconciles the same board immediately, shows stationary short-lived impact/number feedback, and removes cascade travel, scaling, and deliberate pacing.

#### Audio presentation

- The sound palette is mineral magic: short crystal and glass transients communicate board resolution, lower-volume elemental layers identify the dominant cleared gem, stone/metal impacts represent combat, and sparse music supplies atmosphere without masking tactical feedback.
- A resolved clear wave produces one crystal transient, not one sound per cleared cell. Ember, Tide, Venom, and Volt add a fire, water, viscous, or electrical layer respectively. Cascade steps raise the crystal pitch and select progressively brighter variants, capped after step five.
- Special creation/activation layers a medium crystal impact with magic; enemy damage uses varied stone impacts; an enemy response begins with a restrained bell telegraph; removing Frozen may substitute an ice-shatter variant.
- SFX variation is derived from the immutable event sequence and never consumes simulation RNG. Audio therefore observes simulation events without changing deterministic state or replay hashes.
- `music.crystal_cave` loops across title, map, event, rest, reward, summary, and non-boss combat screens. `music.boss_battle` replaces it during a boss encounter. Music changes are presentation-only and do not alter combat timing.
- Eight pooled mono SFX voices prevent pitch changes from affecting an already playing cue. Music uses its own looping stereo source. Device-local SFX and music toggles default on and are stored separately from run checkpoints.
- Imported SFX use mono Vorbis and decompress on load. Music retains stereo, streams from storage, and uses mobile-oriented Vorbis compression.
...
- The CC0 sound-design expansion adds stable `feedback.clear.*`, `feedback.special.*`, `feedback.hit.stone.*`, `feedback.intent`, `feedback.status_removed.frozen.*`, and `music.*` roles. Exact file mappings and voluntary credits remain in the asset ledger.
...
- Session E audio uses pooled one-shot voices plus an independent looping music source. Clear groups, rather than individual `GemCleared` events, are the audible cascade unit; audio variation is presentation-derived and cannot advance authoritative RNG.
...
- Hybrid IDs and generic modifiers are `skill.flashfire` (Ember + Volt; Spark reduces both equipped cooldowns by 1), `skill.galvanic_venom` (Venom + Volt; one Volt-progress point per Poison response tick), `skill.scalding_current` (Ember + Tide; every second Focus conversion empowers the next matched Ember group by +2 damage per gem, with a single-gem fallback for Prism clears), and `skill.toxic_undertow` (Tide + Venom; a Focus conversion adds 1 Toxic at most once per numbered cascade).
- Advanced active IDs are `skill.transmute` (cooldown 5, exactly one movable non-special normal gem plus a normal-color `UseSkillCommand.OptionId`), `skill.detonate` (cooldown 6, exactly one movable match-four special), and `skill.reweave` (cooldown 5, one to three distinct movable non-special normal gems). All three resolve through `BoardSpawn`, emit ordinary clear/special/move/spawn events, and restore the stable playable-board invariant. New board event `GemTransmuted` exposes recolors without bypassing `GemCleared` or `SpecialActivated`.
- The unlocked `event.prismatic_archive` offers `choice.prismatic_archive.study` (lose 6 HP, draft one of three eligible skills) or `choice.prismatic_archive.leave` (heal 3). It joins the seeded event pool only when available in the run's unlock snapshot.

## Changed contracts — R4 mastery
